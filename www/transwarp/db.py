...
def creat_engine(user, passwd, database, host='127.0.0.1', port=3306, **kw):
    import mysql.connector
    global engine
    if engine is not None:
        raise DBError('Engine is already initialized.')
    params = dict(user=user, passwd=passwd, database=database, host=host, port=port)
    defaults = dict(use_unicode=True, charset='utf8', collation='utf8_general_ci', autocomit=False)
...
engine = None
# 定义一个threadlocal对象  任何一个线程都无法访问到其他线程所持有德 数据库连接
_db_ctx = _DbCtx()
...
'''定义__enter__  __exit__是在用with语句，确保任何情况下__exit__（）方法都可以被调用'''
...
if __name__ == '__main__':
    ...
    creat_engine('root','Gfs1655030139', 'test')
